...
1. Identify the affected file and the fix (dependency upgrade, code change, or configuration update).
2. Check out a branch from main.
3. Apply the fix in the correct manifest/source file.
4. Run the affected service's tests.
5. Push the fix.

Group related fixes per service into single PRs when possible. Skip findings in test files.
```
