...
1. **Notification event drift fixed:** The camelCase/snake_case mismatch between `reference/events/schemas/notification-events.json` and the notification-service's actual event publishing code has been resolved. Either the schema was updated to match the service, or the service was updated to match the schema. Check both files for consistency.

2. **Audit event drift fixed:** The missing `timestamp` issue in audit batch events has been resolved. Either `reference/events/schemas/audit-events.json` makes `timestamp` optional for batch events, or the audit-service code now always includes `timestamp`.
...
4. **OpenAPI spec validated:** At least one OpenAPI spec in `reference/openapi/` has been compared against the actual service implementation. Evidence: a contract test was run, or the spec was updated to fix discrepancies, or a comment/doc confirms the spec matches.
