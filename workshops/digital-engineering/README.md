...
This workshop uses **Devin Cloud** via the web app ([app.devin.ai](https://app.devin.ai)). You will paste prompts into Devin, kick off cloud sessions, and review the PRs Devin opens — all from your browser.

> **Tip:** Prefer working in an IDE? The same labs can be run from **Devin Desktop**, where you delegate tasks to Cloud and review PRs with one-click checkout — no browser switching. A terminal-native workflow is also available via **Devin CLI** (`cli.devin.ai`). See the [Desktop + Cloud variant](README.platform.md) for the Desktop-centered delivery.
...
- **Track sessions from the Agent Command Center.** If you have Devin Desktop, open the Agent Command Center to see a Kanban board of your running cloud sessions — handy when you have parallel labs in flight.
...
> **CLI alternative:** This research step can also be done locally with `devin` in your terminal for faster iteration — ask it to analyze CI logs and suggest root causes without spinning up a cloud session.
...
> **Desktop tip:** Use the Agent Command Center in Devin Desktop to monitor all your running sessions on a single Kanban board — especially useful when deploying multiple modules in parallel.
