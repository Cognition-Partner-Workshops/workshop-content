...
- [Choose Your Track](#choose-your-track)
  - [Before You Start: DeepWiki](#before-you-start-deepwiki)
  - [Lab A1 — "Paste a Prompt, Get a PR": Dependency Audit (45 min)](#lab-a1-paste-a-prompt-get-a-pr-dependency-audit-45-min)
  - [Lab A2 — "Devin Runs Your Tools": SAST Scanning with SonarQube (60 min)](#lab-a2-devin-runs-your-tools-sast-scanning-with-sonarqube-60-min)
  - [Lab A3 — "Guardrails That Prevent Future Problems": Secrets Detection (45 min)](#lab-a3-guardrails-that-prevent-future-problems-secrets-detection-45-min)
  - [Lab A4 — "Automate Everything in CI": Security Pipeline (60 min)](#lab-a4-automate-everything-in-ci-security-pipeline-60-min)
  - [Lab A5 — "Devin as a Background Agent": Autonomous SAST Remediation (60 min)](#lab-a5-devin-as-a-background-agent-autonomous-sast-remediation-60-min)
  - [Before You Start: DeepWiki](#before-you-start-deepwiki)
  - [Lab B1 — "Paste a Prompt, Get a PR": Test Coverage Audit (45 min)](#lab-b1-paste-a-prompt-get-a-pr-test-coverage-audit-45-min)
  - [Lab B2 — "Devin Has Its Own Machine": E2E Browser Testing (60 min)](#lab-b2-devin-has-its-own-machine-e2e-browser-testing-60-min)
  - [Lab B3 — "Repetitive Work at Scale": Test Framework Migration (60 min)](#lab-b3-repetitive-work-at-scale-test-framework-migration-60-min)
  - [Lab B4 — "The PR Feedback Loop": BDD Test Generation (60 min)](#lab-b4-the-pr-feedback-loop-bdd-test-generation-60-min)
  - [Lab B5 — "Devin as a Team Member": Continuous Quality Engineering (45 min)](#lab-b5-devin-as-a-team-member-continuous-quality-engineering-45-min)
- [Additional Challenges](#additional-challenges)
  - [Security Extras](#security-extras)
  - [QA Extras](#qa-extras)
- [Repos Required on Devin's Machine](#repos-required-on-devins-machine)
  - [Track A (Security)](#track-a-security)
  - [Track B (QA)](#track-b-qa)
...
```
Resolve this GitHub Issue: https://github.com/Cognition-Partner-Workshops/timesheet-app/issues/2 — audit the npm dependencies for known vulnerabilities, upgrade all vulnerable packages to their latest secure versions, ensure the build and tests still pass.
```
...
Re-run OWASP DC to verify the dependency fixes. Create a `SECURITY_REMEDIATION.md` documenting the before/after results from both tools.
```
...
Create a `COVERAGE_REPORT.md` summarizing: current coverage baseline, files targeted, tests added, and new coverage numbers.
```
...
Write JUnit 5 tests for the 5 least-covered classes, following the existing test patterns (MockMvc for controllers, Mockito for services). Ensure each new test class covers happy path, error cases, and edge cases.
```
...
For each test, take a screenshot at key checkpoints as visual evidence. Run all tests and ensure they pass against the running application. Create a Playwright configuration file if one doesn't exist.
```
...
Use ts-angular-realworld as a reference — it already uses Vitest + Playwright. Create a `MIGRATION_RUNBOOK.md` documenting patterns converted, manual interventions needed, and common gotchas.
```
